module.exports = grammar({
    ...
    rules: {
        module: $ => field("entities", repeat(choice(
            $._decl,
            $.import,
            seq($.attribute, ';'),
            $.statement,
        ...
        params: $ => seq(token('('),
                optional(commaSep1(seq(optional($.inout), $.ident, ':', $.typename, optional(seq('=', $.expression))),)),
                    token(')')),
        ...
        unit_switch_case: $ => prec.right(seq(
        ...
        hook_decl: $ => seq(
            ...
            choice(
                seq(optional(seq(
                    '(',
                    optional(commaSep1(seq($.ident, ':', $.typename))),
                    ')')),
                    $.statement
                ),
                $.foreach,
        ...
        field_decl: $ => prec.right(seq(
            ...
            field("type_", choice($.typename, $.vector, $.regexp, $.bitfield, $.bytes, $.function_call, $.array_access, $.void)),
            field("type_ctor_args", optional(seq("(", optional(commaSep1($.expression)), ")"))),
            optional(token(seq('[', ']'))),
        ...
        while: $ => prec.left(3, seq(
            'while',
            '(',
            choice(
                $.expression,
                seq($.var_decl, $.expression),
            ),
            ')',
            $.statement,
            optional(seq(
        ...
        contains: $ => prec.right(4, seq(
            $.expression,
            'in',
            $.expression,
        )),
        contains_not: $ => prec.right(4, seq(
            $.expression,
            '!in',
            $.expression,
        )),
        ...
        type_member: $ => prec.left(10000, seq(
            $.expression,
            seq(
                '.',
                $.type_member_ident
        ...
        type_member_call: $ => prec.left(1000, seq(
            $.type_member,
            seq('(', optional(commaSep1($.expression)), ')'),
        )),

        array_access: $ => prec(1100, seq(
            $.expression,
            '[',
            $.expression,
            ']',
        ...
        type_member_checked: $ => prec(100, seq(
            $.expression,
            '.?',
            $.ident,
        )),

        type_member_check: $ => prec(100, seq(
            $.expression,
            '?.',
            $.ident,
        )),

        ternary: $ => prec.left(50, seq(
            $.expression,
            '?',
            $.expression,
            ':',
        ...
        dd: _ => '$$',
        self_id: _ => 'self',
        ident: $ => prec.left(seq(/[a-zA-Z_]|[a-zA-Z_][a-zA-Z_0-9]*[a-zA-Z_0-9]/, optional(seq('::', $.ident)))),
        integer: _ => seq(optional('+'), choice(/\d+/, /0x[a-fA-F0-9]+(p\d+)?/)),
        real: _ => seq(
            optional('+'),
            choice(
                /\d+\.\d+/,
                /0x[a-fA-F0-9]+\.[a-fA-F0-9]+(p\d+)?/,
                /\d+[eE]\d+/,
            )
        ),
        ...
        tuple: $ => prec(2, seq('(', optional(commaSep1($.expression)), optional(','), ')')),
        map: $ => seq(
            'map',
            '(',
